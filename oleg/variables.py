EMOJI_POOL_GUILD_ID = 1116299975870136330
GENERAL_GUILD_ID = 1130589089658306672
RUSSIAN_ROLE_ID = 1174034977504108675
ENGLISH_ROLE_ID = 1174035039936335982
RUSSIAN_CHANNEL_ID = 1130594736944726106
ENGLISH_CHANNEL_ID = 1174058375521239070
JOIN_MEMBER_EMOJI_ID = 1174228725857005629

RUSSIAN_WELCOMES = [
    "{}, Добро пожаловать!",
    "{}, Теперь наш друг!",
    "{}, Надеюсь ты к нам не без пиццы.",
    "{}, Пришел на наш замечательный сервер!",
    "{}, Надеюсь, что тебе тут понравится и проведешь время с удовольствием!",
    "{}, Врывается в наш клуб.",
    "{}, Welcome to the club buddy.",
    "{}, Вы попали в анти-пространство.",
    "{} — Самое умное существо в мире, поскольку оно прошло верификацию.",
    "{}, А ты мне нравишься!",
    "{}, Ничего себе! Ты так изменился, а может, я просто давно тебя не видел!",
    "{}, Мне кажется, или я реально тебя вижу!",
    "{}, О, здравствуй, мой драгоценный друг!",
    "{}, Стоп! Стоп! Стоп! Дай я тебя сфотографирую, иначе никто не поверит, что я тебя видел!",
    "{}, Какие люди! Рад вас видеть вот тут, просто так, как живого человека… я-то думал вы – призрак!",
    "{}, Ты ли это? Я не верю своим глазам и ушам!",
    "{}, Сколько лет и сколько зим! ТЫ всегда неотразим!",
    "{}, Здравствуй! Видеть твою улыбку – огромное счастье для меня сегодня!",
    "{}, Дружище! Вот это поворот! Как же так? Года идут, а ты не меняешься! Привет!",
    "{}, Здрямс!",
    "{}, Йо!",
    "{}, Хеллоу!",
    "{}, Бонжур!",
]

ENGLISH_WELCOMES = [
    "Welcome, {}!",
    "Welcome, {}! Now you're our friend!",
    "{}, I hope you didn't come here without pizza.",
    "Welcome, {}! You've joined our wonderful server!",
    "{}, I hope you enjoy your time here!",
    "{}, crashing into our club.",
    "Welcome to the club buddy, {}.",
    "{}, you have entered the anti-space.",
    "{} - the smartest creature in the world, because it has been chosen language.",
    "{}, I like you!",
    "{}, Wow! You've changed so much, or maybe I just haven't seen you in a long time!",
    "{}, I think I really see you!",
    "Oh, hello, my precious friend, {}!",
    "{}, Stop! Stop! Stop! Let me take a picture of you, otherwise no one will believe that I saw you!",
    "What people! {}, I'm glad to see you here, just like a living person... I thought you were a ghost!",
    "{}, Is that you? I can't believe my eyes and ears!",
    "{}, it's been ages! YOU're always irresistible!",
    "Hello! Seeing your smile is a huge happiness for me today, {}!",
    "Buddy! This is a twist! How can this be? Years go by, but you stay the same! Hello!",
    "{}, Sup!",
    "{}, Yo!",
    "{}, Hello!",
    "{}, Bonjour!",
]
